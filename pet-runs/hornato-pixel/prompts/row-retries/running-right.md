Create Codex pet row `running-right` for `hornato-pixel`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Same Hornato identity: round bright-red tomato mascot, one tiny devil horn on its left side only, cute face, small leaf stem on top, no other horns. Render as readable pixel art with chunky silhouette and limited palette.. Preserve silhouette, face, palette, material, proportions, markings, and props.
